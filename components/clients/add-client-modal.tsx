...
interface AddClientModalProps {
  ...
  onConfirm: (data: ClientFormData) => Promise<boolean> | boolean
...
export function AddClientModal({ open, onOpenChange, onConfirm, defaultValues }: AddClientModalProps) {
  ...
  const onSubmit = async (data: ClientFormData) => {
    ...
    try {
      ...
      // only close modal if caller indicates success (true)
      if (result === true) {
        onOpenChange(false)
      }
    } catch (e) {
      // if onConfirm throws, show a toast and keep modal open so user can fix
      try {
        showToast('error', 'Erro ao salvar', { description: String((e as any)?.message ?? e) })
      } catch (_) {
        // ignore
      }
    }
  ...
  const referredById = watch('referredBy')
  ...
  return (
    <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <div className="text-sm">{referredByName ?? (referredById ? referredById : 'Nenhum selecionado')}</div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" type="button" onClick={() => setOpenSearchClient(true)}>Selecionar</Button>
                      <Button size="sm" variant="ghost" type="button" onClick={() => { setValue('referredBy', undefined); setReferredByName(null) }}>Limpar</Button>
                    </div>
                  </div>
      ...
      {openSearchClient && (
        <SearchClientTable
          open={openSearchClient}
          onOpenChange={setOpenSearchClient}
          onSelect={(client: any) => {
            // set the referredBy id and label
            setValue('referredBy', client.id as any)
            setReferredByName(client.name)
